...
def run_simulation(source_type):
    if source_type == "current":
        # Run Elmer simulations for current
        subprocess.run(["ElmerSolver", "current_stranded1.sif"], check=True)
        subprocess.run(["ElmerSolver", "current_stranded10.sif"], check=True)

        # Check if simulation results exist, then plot
        if os.path.isdir("results_stranded1") and os.path.isdir("results_stranded10"):
        ...
        # Run Elmer simulations for voltage
        subprocess.run(["ElmerSolver", "voltage_stranded1.sif"], check=True)
        subprocess.run(["ElmerSolver", "voltage_stranded10.sif"], check=True)
